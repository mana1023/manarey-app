...
class VentaDetalleDialog(QDialog):
    ...
    def __init__(self, venta_id: int, parent=None):
        ...
        self.resize(700, 600)
        self.setStyleSheet(
            f"QDialog{{background:{_T('CARD', '#232327')};}} QLabel{{color:{_T('TEXT', '#ECECF1')};}}"
        )
    ...
    def load_and_display(self):
        ...
        info_frame.setStyleSheet(
            f"background:{_T('BG', '#1f1f22')};border:1px solid {_T('BORDER', '#34343a')};border-radius:10px;padding:12px;"
        )
        ...
        cliente_title.setStyleSheet(f"color:{_T('GOLD', '#C9A040')};")
        ...
        productos_title.setStyleSheet(f"color:{_T('GOLD', '#C9A040')};")
        ...
        productos_table.setStyleSheet(
            f"""
            QTableWidget {{
                background:{_T('BG', '#1f1f22')};
                color:{_T('TEXT', '#ECECF1')};
                gridline-color:{_T('BORDER', '#34343a')};
                border:1px solid {_T('BORDER', '#34343a')};
                border-radius:8px;
            }}
            QHeaderView::section {{
                background:{_T('BG', '#1f1f22')};
                color:{_T('GOLD', '#C9A040')};
                font-weight:700;
                padding:8px;
                border:none;
            }}
        ...
        total_label.setStyleSheet(f"color:{_T('GOLD', '#C9A040')};")
        ...
        close_btn.setStyleSheet(
            f"QPushButton{{background:{_T('BORDER', '#34343a')};color:{_T('TEXT', '#ECECF1')};border:none;"
            "border-radius:10px;padding:10px 20px;font-weight:700;}}"
        )
    ...
    def create_info_label(self, label: str, value: str):
        ...
        widget.setStyleSheet(f"color:{_T('TEXT', '#ECECF1')};")
        return widget
...
class HistorialVentasView(QMainWindow):
    ...
    def __init__(self, username: str, role: str, local: str, back_command=None):
        ...
        try:
            import app_theme as _at_hv

            self.setStyleSheet(
                _at_hv.build_stylesheet(
                    _at_hv.is_dark_mode(), _at_hv.get_font_size_px()
                )
            )
        except Exception:
            self.setStyleSheet(
    ...
    def setup_ui(self):
        ...
        if self.back_command:
            ...
            try:
                import app_theme as _at_bk2

                _c_bk2 = _at_bk2._palette(_at_bk2.is_dark_mode())
                back_btn.setStyleSheet(
                    f"QPushButton{{background:{_c_bk2['BG_ALT']};color:{_c_bk2['GOLD']};"
                    f"border:1px solid {_c_bk2['BORDER']};border-radius:10px;padding:8px 14px;font-weight:700;}}"
                    f"QPushButton:hover{{background:{_c_bk2['SURFACE']};border-color:{_c_bk2['GOLD']};}}"
                )
            except Exception:
                back_btn.setStyleSheet(
    ...
    def create_stats_section(self, layout):
        ...
        stats_frame.setStyleSheet(
            f"""
            QFrame{{background:{_T('CARD', '#232327')};border:1px solid {_T('BORDER', '#34343a')};border-radius:12px;padding:16px;}}
        """
        )
    ...
    def create_stat_card(self, title: str, value: str):
        ...
        title_label.setStyleSheet(
            f"color:{_T('TEXT_MUTED', '#c9c9cf')};font-size:11px;"
        )
        card_layout.addWidget(title_label)

        value_label = QLabel(value)
        value_label.setFont(QFont("Segoe UI", 18, QFont.Black))
        value_label.setStyleSheet(f"color:{_T('GOLD', '#C9A040')};")
    ...
    def create_table_section(self, layout):
        ...
        self.table.setStyleSheet(
            ...
            QHeaderView::section {{
                ...
                padding:10px;
                border:none;
                border-bottom:2px solid {_T('GOLD', '#C9A040')};
            }}
        """
        )
    ...
    def refresh_theme(self):
        try:
            ...
            self.setStyleSheet(
                _at_rt.build_stylesheet(
                    _at_rt.is_dark_mode(), _at_rt.get_font_size_px()
                )
            )
        except Exception:
            pass
