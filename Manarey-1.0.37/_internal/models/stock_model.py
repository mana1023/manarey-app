...
def _touch_update(conn, pid: int) -> None:
    try:
        conn.execute(
            "UPDATE productos SET updated_at=? WHERE id=?", (_now_local(), pid)
        )
    ...
    try:
        ...
        with _get_conn_cm() as conn:
            ...
            cur.execute(
                ...
                WHERE local = {ph} AND LOWER(nombre)=LOWER({ph}) AND COALESCE(is_combo,0)=1
                """,
                (local, nombre),
            )
            if cur.fetchone():
                return False, "Ya existe un combo con ese nombre", None
            ...
            if isinstance(conn, sqlite3.Connection):
                cur.execute(
                    ...
                    (nombre, categoria, medida, estado, color, cantidad, precio_costo, precio_venta, local, created_at, updated_at, is_combo)
                    VALUES (?,?,?,?,?,?,?,?,?,?,?,1)
                    """,
                    (
                        nombre,
...
def update_combo(
    ...
    try:
        ...
        with _get_conn_cm() as conn:
            ...
            ph = "?" if isinstance(conn, sqlite3.Connection) else "%s"
            cur.execute(
                f"SELECT nombre FROM productos WHERE id={ph} AND local={ph} AND COALESCE(is_combo,0)=1",
                (int(combo_producto_id), local),
            )
            row = cur.fetchone()
            if not row:
            ...
            cur.execute(
                f"UPDATE productos SET nombre={ph}, precio_venta={ph}, updated_at={ph} WHERE id={ph}",
                (nombre, float(precio_venta), _now_local(), int(combo_producto_id)),
            )
            cur.execute(
                f"DELETE FROM combo_items WHERE combo_producto_id={ph}",
                (int(combo_producto_id),),
...
def sync_combo_across_locals(
    ...
    with _get_conn_cm() as conn:
        ...
        ph = "?" if isinstance(conn, sqlite3.Connection) else "%s"
        cur.execute(
            f"SELECT nombre, COALESCE(precio_venta,0) FROM productos WHERE id={ph}",
            (int(combo_producto_id),),
        )
        row = cur.fetchone()
        if not row:
        ...
        for loc in locals_list:
            ...
            cur.execute(
                f"""
                SELECT id FROM productos
                WHERE local={ph} AND COALESCE(is_combo,0)=1 AND (LOWER(nombre)=LOWER({ph}) OR LOWER(nombre)=LOWER({ph}))
                LIMIT 1
                """,
                (loc, combo_name, (old_name or combo_name)),
            )
            ...
            if target_combo_id:
                cur.execute(
                    f"UPDATE productos SET nombre={ph}, precio_venta={ph}, updated_at={ph} WHERE id={ph}",
                    (combo_name, combo_price, now, target_combo_id),
                )
                cur.execute(
                    f"DELETE FROM combo_items WHERE combo_producto_id={ph}",
                    (target_combo_id,),
...
def sync_combos_for_local(local: str) -> None:
    ...
    with _get_conn_cm() as conn:
        ...
        combos = cur.fetchall()
        for combo_id, combo_name, combo_price in combos:
            ...
            cur.execute(
                ...
                WHERE local={ph} AND COALESCE(is_combo,0)=1 AND LOWER(nombre)=LOWER({ph})
                LIMIT 1
                """,
                (local, combo_name),
            )
...
def get_stock_filtered(
    ...
    try:
        try:
            ...
            pass
        # Normalizar duplicados antes de devolver el listado para evitar filas repetidas en la UI.
        merge_duplicate_products(local)
        rows = list_by_local(
            local, search, categoria, estado, medida, fabricante, color
        )
...
def merge_duplicate_products(local: str) -> int:
    """
    Fusiona productos que son exactamente iguales (mismo nombre, categoría,
    medida, estado, color, fabricante y precio de venta) sumando sus cantidades y
        ...
        try:
            with _get_conn_cm() as conn:
                ...
                try:
                    cur.execute(
                        f"""
                        SELECT id, nombre, {material_col}categoria, COALESCE(medida,''), estado,
                               COALESCE(color,''), COALESCE(fabricante,''), COALESCE(codigo,''),
                               COALESCE(precio_venta,0), COALESCE(precio_costo,0), COALESCE(cantidad,0),
                               COALESCE(is_combo,0)
                        FROM productos
                        WHERE local = {ph}
                        ORDER BY id ASC
                    ...
                    try:
                        cur.execute(
                            f"""
                            SELECT id, nombre, {material_col}categoria, COALESCE(medida,''), estado,
                                   COALESCE(color,''), COALESCE(fabricante,''), COALESCE(precio_venta,0),
                                   COALESCE(precio_costo,0), COALESCE(cantidad,0), COALESCE(is_combo,0)
                            FROM productos
                            WHERE local = {ph}
                            ORDER BY id ASC
                        """,
                        ...
                        cur.execute(
                            ...
                            SELECT id, nombre, {material_col}categoria, COALESCE(medida,''), estado,
                                   COALESCE(color,''), COALESCE(fabricante,''), COALESCE(precio_venta,0),
                                   COALESCE(precio_costo,0), COALESCE(cantidad,0)
                            FROM productos
                            WHERE local = {ph}
                            ORDER BY id ASC
                        """,
                ...
                if has_material and has_codigo:
                    qty_index = 11
                elif has_material and not has_codigo:
                    qty_index = 10
                elif has_codigo:
                    qty_index = 10
                else:
                ...
                for r in rows:
                    if has_codigo:
                        if has_material:
                            ...
                            combo_index = 11
                        if has_combo:
                            base = base + (int(r[combo_index] or 0),)
                        key = base
                    else:
                        if has_material:
                            base = (
                            ...
                            base = (r[1], r[2], r[3], r[4], r[5], r[6], int(r[7] or 0))
                            combo_index = 10
                        if has_combo:
                            base = base + (int(r[combo_index] or 0),)
                        key = base
                ...
                for items in groups.values():
                    if len(items) <= 1:
                        continue
                    keep_id = int(items[0][0])
                    total_qty = sum(int(it[qty_index] or 0) for it in items)
                    # Actualizar cantidad en el producto que se conserva
                    cur.execute(
                        f"UPDATE productos SET cantidad={ph}, updated_at={ph} WHERE id={ph}",
                        (total_qty, now, keep_id),
                    )
                    # Eliminar duplicados restantes
                    ids_to_delete = [int(it[0]) for it in items[1:]]
...
def list_by_local(
    ...
    try:
        with _get_conn_cm() as conn:
            ...
            if search:
                where.append(f"LOWER(nombre) LIKE LOWER({ph})")
            ...
            if color:
                where.append(f"COALESCE(color,'') = COALESCE({ph},'')")
                params.append(color)
            has_material = False
            try:
                if isinstance(conn, sqlite3.Connection):
...
def change_state_quantity(
    ...
    try:
        ...
        try:
            ...
            logger.error(f"No se pudo fusionar duplicados tras cambio de estado: {_e}")
