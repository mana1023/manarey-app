...
class ProductSelectionDialog(QDialog):
    ...
    def __init__(self, local: str, parent=None, initial_cart=None):
        ...
        self.setModal(True)
        # Bug fix: el padre (BoletaView) es fullscreen frameless — sin
        # WindowStaysOnTopHint el dialog queda detrás y parece que "no aparece".
        try:
            self.setWindowFlags(
                self.windowFlags() | Qt.Dialog | Qt.WindowStaysOnTopHint
            )
        except Exception:
            pass
    ...
    def _load_size_pref(self):
        ...
        try:
            ...
            if screen:
                ...
                max_w = max(default_w, min(int(geo.width() * 0.72), geo.width() - 120))
                max_h = max(default_h, min(int(geo.height() * 0.82), geo.height() - 80))
        except Exception:
            pass
        try:
        ...
        try:
            ...
            if os.path.exists(PREFS_PATH):
                ...
                if size and isinstance(size, dict):
                    saved_w = int(size.get("w", default_w) or default_w)
                    saved_h = int(size.get("h", default_h) or default_h)
                    self.resize(
                        max(min_w, min(saved_w, max_w)),
                        max(min_h, min(saved_h, max_h)),
                    )
                    return
        except Exception:
            pass
        # Asegurarse de no solicitar un tamaño mayor al máximo permitido
        self.resize(min(default_w, max_w), min(default_h, max_h))
    ...
    def _save_size_pref(self):
        try:
            ...
            if os.path.exists(PREFS_PATH):
                data = json.load(open(PREFS_PATH, "r", encoding="utf-8")) or {}
            size = self.size()
            save_w = int(size.width())
            save_h = int(size.height())
            try:
                screen = self.screen() or QGuiApplication.primaryScreen()
                if screen:
                    geo = screen.availableGeometry()
                    save_w = max(
                        min_w, min(save_w, max(min_w, int(geo.width() * 0.46)))
                    )
                    save_h = max(
                        min_h, min(save_h, max(min_h, int(geo.height() * 0.66)))
                    )
            except Exception:
                pass
            data["product_selector_size"] = {"w": save_w, "h": save_h}
    ...
    def _center_over_parent(self):
        try:
            ...
            target_top = max(parent_geo.top() + 24, dialog_geo.top())
            self.move(dialog_geo.left(), target_top)
        except Exception:
            pass
    ...
    def _stop_load_thread(self):
        """Detiene el QThread de carga de productos de forma segura.
        Sin esto, si el usuario cancela mientras el thread corre, Qt aborta con
        'QThread: Destroyed while thread is still running' → crash de la app.
        """
        try:
            t = getattr(self, "_load_thread", None)
            if t is None:
                return
            # Verificar que el objeto C++ aún exista antes de tocarlo
            try:
                if sip.isdeleted(t):
                    self._load_thread = None
                    return
                ...
                pass
            # Desconectar el slot propio para evitar que el thread llame métodos
            # del dialog una vez que éste ya se está destruyendo.
            try:
                t.data_loaded.disconnect(self._on_products_loaded)
            except Exception:
                pass
            running = False
            try:
            ...
            if running:
                try:
                    ...
                    pass
                # Si sigue ocupado en IO, desacoplarlo del dialog para que Qt
                # no destruya un hilo todavia en ejecucion.
                if not t.wait(3000):
                    try:
                        t.setParent(None)
                ...
                # El thread ya terminó: destruirlo manualmente ya que
                # quitamos finished.connect(deleteLater) de load_products.
                try:
                    t.deleteLater()
                except Exception:
                    pass
    ...
    def setup_ui(self):
        ...
        try:
            ...
            self.table.setStyleSheet(selector_style)
        except Exception:
            selector_style = (
            ...
            self.table.setStyleSheet(selector_style)
        # Placeholders invisibles — el código los referencia pero ya no se muestran.
        # La tabla siempre está visible; los productos aparecen cuando llegan del DB.
        self._center_panel = QWidget()
        self._center_panel.setVisible(False)
        self._loading_label = QLabel()
        self._loading_label.setVisible(False)
        self._loading_bar = QProgressBar()
        self._loading_bar.setVisible(False)
        self._empty_table_label = QLabel()
        self._empty_table_label.setVisible(False)
        self._selector_table_style = (
            selector_style.replace("{", "{{").replace("}", "}}")
            + "QTableView{font-size:{font}px;}"
            + "QHeaderView::section{font-size:{header_font}px;padding:{header_pad}px;}"
            + "QTableView::item{padding:{item_pad_v}px {item_pad_h}px;}"
        )
        ...
        # _center_panel ya no va en el layout — la tabla siempre visible
    ...
    def _set_loading(self, loading: bool):
        """Actualiza el flag de carga. La tabla siempre está visible."""
    ...
    def load_products(self):
        try:
            ...
            self._pending_local = selected_local
            self._load_counter += 1
            self._current_load_id = self._load_counter

            # ── Caché de sesión: mostrar datos anteriores de inmediato ──────────
            # Si ya cargamos este local antes, mostramos esos datos al instante
            # y lanzamos el refresh en background (sin spinner).
            # La primera vez no hay caché → spinner normal.
            cache_key = (selected_local or "__all__").lower()
            cached = _dialog_product_cache.get(cache_key)
            if cached:
                self.all_products = list(cached)
                self.products_data = {p["id"]: p for p in self.all_products}
                self._set_loading(False)
                self.filter_products()
                # Refresh en background sin mostrar spinner
                self._start_load_thread(selected_local, self._current_load_id)
            else:
                # Sin caché: spinner hasta que lleguen los datos reales de la DB.
                # No se muestra el offline porque tiene cantidades desactualizadas.
                self._set_loading(True)
                self._start_load_thread(selected_local, self._current_load_id)
                load_id_snap = self._current_load_id

                def _loading_timeout():
                    try:
                        if (
                            getattr(self, "_is_loading", False)
                            and getattr(self, "_current_load_id", None) == load_id_snap
                        ):
                            logger.warning(
                                "load_products timeout — forzando _set_loading(False)"
                            )
                            self._set_loading(False)
                            self.filter_products()
                    except Exception:
                ...
                QTimer.singleShot(20_000, _loading_timeout)
        except Exception as e:
            self._set_loading(False)
            QMessageBox.critical(self, "Error", f"Error cargando productos: {e}")

    def _start_load_thread(self, local, load_id):
        """Inicia ProductLoadWorker para cargar productos sin bloquear UI.
        Usa el worker robusto con múltiples fallbacks (psycopg2 → pool → get_stock_filtered
        → SQLite → caché offline) y timeout de 10 s en lugar del inline loader anterior
        que tenía timeout de 3 s y fallaba silenciosamente.
        """
        # Detener thread anterior si sigue corriendo
        ...
        self._load_thread = t
        t.data_loaded.connect(self._on_products_loaded)
        t.start()

    def _on_products_loaded(self, load_id, products):
        """Handler cuando los productos se cargan en el thread."""
        try:
            if load_id != self._current_load_id:
                # Carga obsoleta: si el spinner sigue visible lo ocultamos igual
                if getattr(self, "_is_loading", False):
                    self._set_loading(False)
                return

            self._is_loading = False
            # Ocultar spinner lo antes posible; filter_products también lo hace
            # pero si algo falla antes de llamarlo, el spinner quedaría visible.
            try:
                self._loading_label.setVisible(False)
                self._loading_bar.setVisible(False)
            except Exception:
            ...
            # Actualizar caché de sesión si llegaron datos reales
            if products:
            ...
            self._is_loading = False
            self._set_loading(False)
            QMessageBox.critical(self, "Error", f"Error procesando productos: {e}")

    def on_search_changed(self, text=""):
        """Maneja cambios en el campo de búsqueda, incluyendo limpiar con la X"""
        # text puede llegar vacío desde returnPressed (que no emite argumento)
        self.filter_products()
    ...
    def filter_products(self):
        try:
            ...
            fabricante = ""
            try:
                fab_widget = getattr(self, "fabricante_combo", None)
                if fab_widget:
                    fabricante = (fab_widget.currentText() or "").strip()
                    if fabricante == "Fabricante":
                        fabricante = ""
            except Exception:
                fabricante = ""

            # Medidas (plaza / rodado / medida libre)
            medidas = []
            try:
                if (
            ...
            # Aplicar filtros sobre all_products
            ...
            if medidas:
                medidas_norm = [m.strip().lower() for m in medidas if m]
                filtered = [
                    p
                    for p in filtered
                    if (p.get("medida", "") or "").strip().lower() in medidas_norm
                ]

            # Actualizar modelo — la tabla siempre está visible
    ...
    def increment_quantity(self, product_id):
        ...
        if not product:
            # Buscar en all_products como fallback
            product = next(
                (p for p in (self.all_products or []) if p.get("id") == product_id),
                None,
            )
        if not product:
            return
        ...
        # Solo bloquear si hay stock registrado (>0) y ya se alcanzó el límite.
        # Productos con stock=0 se consideran sin restricción de cantidad.
        if stock > 0 and current_qty >= stock:
            if self._should_warn_insufficient(product_id):
                QMessageBox.warning(
                    self,
                    "Stock insuficiente",
                    f"No hay más stock disponible. Máximo: {stock}",
                )
            return
