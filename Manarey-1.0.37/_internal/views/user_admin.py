...
class UserEditDialog(QDialog):
    def __init__(self, parent=None, user=None):
        ...
        info = QLabel(
            "Aqu? pod?s crear usuarios nuevos, cambiar contrase?as, asignar roles\n"
        ...
        self.password.setPlaceholderText("m?nimo 6 caracteres")

        self.password_confirm = QLineEdit()
        self.password_confirm.setEchoMode(QLineEdit.Password)
        self.password_confirm.setPlaceholderText("repetir contrase?a")

        self.role = QComboBox()
        self.role.addItems(["admin", "local"])
        self.local = QLineEdit()
        self.local.setPlaceholderText("ej: Vidriera, Dep?sito, Local 2")

        layout.addRow("Usuario", self.username)
        layout.addRow("Contrase?a (dejar en blanco para no cambiar)", self.password)
        layout.addRow("Confirmar contrase?a", self.password_confirm)

        show_pass = QCheckBox("Mostrar contrase?a")
...
class UserAdminWindow(QMainWindow):
    def __init__(self, parent=None, back_command=None):
        ...
        head = QLabel("Gesti?n de usuarios")
        head.setStyleSheet("font-size:20px; font-weight:800;")
        head_row.addWidget(head)
        head_row.addStretch()
        v.addLayout(head_row)
        subtitle = QLabel("Crear usuarios, cambiar contrase?as, roles y locales.")
        subtitle.setStyleSheet("color:#9ca3af; font-size:12px;")
        v.addWidget(subtitle)

        helper = QFrame()
        helper.setStyleSheet(
            "QFrame{background:#111827;border:1px solid #1f2937;border-radius:10px;}"
        )
        helper_lay = QHBoxLayout(helper)
        helper_lay.setContentsMargins(12, 10, 12, 10)
        helper_text = QLabel(
            "Tip: Seleccion? un usuario en la tabla para editar su rol, local o contrase?a."
        )
        ...
        btn_edit = QPushButton("?? Editar usuario")
        btn_delete = QPushButton("?? Eliminar usuario")
        ...
        locales_frame.setStyleSheet(
            "QFrame{background:#0f172a;border:1px solid #1f2937;border-radius:10px;}"
        )
