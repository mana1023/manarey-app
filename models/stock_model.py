...
def ensure_combo_schema() -> Tuple[bool, str]:
    ...
    Optimización crítica de rendimiento: en PostgreSQL verifica primero en
    information_schema si la migración ya está completa. ALTER TABLE necesita
    ACCESS EXCLUSIVE lock y bloquea TODAS las queries de la tabla mientras
    espera — en instalaciones ya migradas esto causaba bloqueos de 4-30s en
    cada arranque. La consulta a information_schema es una vista del sistema
    que no genera ningún lock y retorna en <5ms.
    """
    try:
        ...
        with _COMBO_SCHEMA_LOCK:
            ...
            with _get_conn_cm() as conn:
                ...
                if _is_postgres():
                        ...
                        SELECT
                          (SELECT 1 FROM information_schema.columns
                           WHERE table_schema='public'
                             ...
                             AND column_name='is_combo') AS has_is_combo,
                          (SELECT 1 FROM information_schema.tables
                           WHERE table_schema='public'
                             AND table_name='combo_items') AS has_combo_items
                    """
                    )
                    row = cur.fetchone()
                    if row and row[0] and row[1]:
                        # Schema ya completo — no ejecutar ningún DDL costoso
                        _COMBO_SCHEMA_OK = True
                        return True, "OK"
                    # Primera vez: schema incompleto → ejecutar migraciones
                    cur.execute(
                    ...
                    # SQLite: verificar por PRAGMA (no tiene information_schema)
                    ...
                    for col_name in (
                        ...
                        if col_name not in ci_cols:
                            cur.execute(
                                f"ALTER TABLE combo_items ADD COLUMN {col_name} TEXT"
                            )
                    cur.execute(
                        "CREATE INDEX IF NOT EXISTS idx_combo_items_combo ON combo_items(combo_producto_id)"
                    )
...
def insert_tipo_si_no_existe(categoria: str) -> None:
    """Registra una categoría nueva si todavía no aparece en ningún producto.

    Las categorías en Manarey son texto libre en la columna `productos.categoria`
    — no existe una tabla separada de tipos.  Esta función es un no-op que se
    mantiene por compatibilidad con llamadas heredadas en add/edit de productos.
    La categoría quedará disponible en `get_all_tipos()` en cuanto el producto
    se guarde en la BD.
    """
    return  # No hay tabla de tipos separada; la categoría se guarda con el producto
...
def _unaccent(text: str) -> str:
    """Quita tildes para búsquedas sin acento."""
    try:
        return (
...
def _py_search_rows(rows: list, search: str) -> list:
    """Filtra filas por búsqueda de texto con soporte de tildes.

    Busca en nombre (índice 1) y en el resto de campos de texto de la fila.
    """
    if not rows or not search:
        return rows
    norm = _unaccent(search).lower()
    terms = [t for t in norm.split() if t]
    if not terms:
        return rows
    result = []
    for row in rows:
        # Construir cadena buscable con todos los campos de texto de la fila
        searchable = _unaccent(
            " ".join(str(v) for v in row if isinstance(v, str))
        ).lower()
    ...
    try:
        with _get_conn_cm() as conn:
            ...
            if local and local not in ("Todos", "Todos los locales"):
                ...
                params.append(local)
            # Búsqueda de texto: en PostgreSQL se aplica con ILIKE directamente en SQL
            # para reducir el volumen de datos antes de fetchall.  En SQLite se deja
            # para _py_search_rows (Python) porque LIKE no soporta tildes.
            _sql_search_applied = False
            if search and not isinstance(conn, sqlite3.Connection):
                _terms = [t.strip() for t in search.split() if t.strip()]
                for _term in _terms:
                    _concat = (
                        "LOWER(COALESCE(nombre,'') || ' ' || COALESCE(material,'') || ' ' || "
                        "COALESCE(categoria,'') || ' ' || COALESCE(color,'') || ' ' || "
                        "COALESCE(codigo,'') || ' ' || COALESCE(fabricante,''))"
                    )
                    where.append(f"{_concat} LIKE LOWER({ph})")
                    params.append(f"%{_term}%")
                _sql_search_applied = True
            if categoria:
            ...
            # Límite de seguridad: evita traer filas ilimitadas cuando el catálogo crece.
            # Se puede subir con MANAREY_MAX_STOCK_ROWS; 0 = sin límite (no recomendado).
            try:
                _max_rows = int(os.environ.get("MANAREY_MAX_STOCK_ROWS", "5000"))
            except Exception:
                _max_rows = 5000
            _limit_clause = f"LIMIT {_max_rows}" if _max_rows > 0 else ""
            q = f"""
                SELECT {cols_with_fab_codigo}
                FROM productos
                {where_clause}
                ORDER BY CASE WHEN COALESCE(cantidad,0) > 0 THEN 0 ELSE 1 END, nombre ASC
                {_limit_clause}
            """
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        rows = cur.fetchall()
            if rows:
                # Si la búsqueda ya se filtró en SQL (ILIKE), _py_search_rows
                # sólo corre como refinamiento de tildes (no vuelve a filtrar).
                # En SQLite siempre corre porque LIKE no soporta diacríticos.
                if _sql_search_applied and not isinstance(conn, sqlite3.Connection):
                    return _py_search_rows(rows, search)
                return _py_search_rows(rows, search)
            ...
            try:
                ...
                return _py_search_rows(rows2, search)
            except Exception:
                return _py_search_rows(rows, search)
    except Exception as e:
