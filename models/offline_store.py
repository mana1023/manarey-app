...
def get_cached_stock(
    ...
    for p in rows:
        if s:
            lowname = (p.get("nombre") or "").lower()
            if not (
                s in lowname
                or s in (p.get("categoria") or "").lower()
                or s in (p.get("fabricante") or "").lower()
                or s in (p.get("color") or "").lower()
                or s in (p.get("codigo") or "").lower()
            ):
                continue
        ...
        if col and col != (p.get("color") or "").lower():
            continue
        if cod and cod not in (p.get("codigo") or "").lower():
            continue
        if med_list:
